...
import {
  TurnstileDeviceEntity,
  TurnstileDirection,
} from '../database/entities/turnstile-device.entity';
import {
  AccessEventEntity,
  AccessEventType,
...
export class TurnstileService {
  ...
  async createDevice(data: {
    ...
    return this.devicesRepo.save(this.devicesRepo.create({ ...data, ip }));
  }
  ...
  async recordIngestEvent(data: {
    ...
    const snapshotUrl = await this.camera.captureForAccessEvent(eventType);
    if (snapshotUrl) {
      ev.snapshotUrl = snapshotUrl;
      await this.accessRepo.save(ev);
    }
  ...
  async createAccessEvent(data: {
    ...
    if (!deviceId && data.deviceIp?.trim()) {
      const d = await this.devicesRepo.findOne({
        where: { ip: normalizeIp(data.deviceIp) },
      });
  ...
  async updateAccessEvent(
    ...
    if (data.rawCardId !== undefined)
      ev.rawCardId = data.rawCardId?.trim() || null;
